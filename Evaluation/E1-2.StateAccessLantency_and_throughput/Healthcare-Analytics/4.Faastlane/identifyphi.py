...
def extract_entities_from_message(message):
    ...
    if not redis_client.exists(message[:10]):
        client = boto3.client(service_name='comprehendmedical',
                              aws_access_key_id="AKIASAP6ZNDDJS52XWPM",
                              aws_secret_access_key="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",
...
def main(event):
    ...
    result = {"data": json.dumps({"message": message, "entities": response['Entities']})}
